...
const ChannelContainer = ({ logo, name, previousImpressions, currentImpressions, CPM, YoY, Benchmark, budget, spent, startDate, endDate }) => {
  const [isOpen, setIsOpen] = useState(false);
  const percentageSpent = ((spent / budget) * 100).toFixed(0);
  const start = new Date(startDate);
  const end = new Date(endDate);
  const flightDuration = !isNaN(start) && !isNaN(end) ? Math.ceil((end - start) / (1000 * 60 * 60 * 24)) : "N/A";
  ...
  return (
    <div className={`channel__container${isOpen ? " open" : ""}`}>
      <div className="channel__stats__container desktop">
        <div className="channel__title__container">
          {logo && <img className="channel__logo" src={logo} alt={`${name} logo`} />}
          <h4>{name}</h4>
        </div>
        <div className="channel__impressions">
          <img
            src={Number(previousImpressions) > Number(currentImpressions) ? "/IMPRESSIONS_TRIANGLE_BAD.svg" : "/IMPRESSIONS_TRIANGLE_GOOD.svg"}
      ...
      <div className="channel__stats__container mobile">
        <div className="channel__title__container">
          {logo && <img className="channel__logo" src={logo} alt={`${name} logo`} />}
          <h4>{name}</h4>
        </div>
        <div className="channel__content__container">
        <div className="channel__values__container names">
          {" "}
        ...
        <div className="channel__values__container values">
          <div className="channel__impressions">
            <img
              src={Number(previousImpressions) > Number(currentImpressions) ? "/IMPRESSIONS_TRIANGLE_BAD.svg" : "/IMPRESSIONS_TRIANGLE_GOOD.svg"}
              alt="Impressions Triangle"
